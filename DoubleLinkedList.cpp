...
void removePos(d_List*& L, int data, int pos)
{
    if (pos<0 || pos>countElements(L) - 1) {
        return;
    }
    else if (pos == 0) {
        removeHead(L);
    }
    else if (pos == countElements(L) - 1) {
        removeTail(L);
    }
    else {
        d_NODE* curr = L->pHead;
        ...
        if (curr->key != data) {
            cout << "Tai vi tri pos khong phu hop voi gia tri bang data";
        }
        else {
            d_NODE* tmp = curr->pNext;
            prev->pNext = tmp;
            tmp->pPrev = prev;
            curr->pNext = NULL;
            curr->pPrev = NULL;
...
d_List* reverseList(d_List* L)
{
    if (L->pHead == NULL && L->pTail==NULL) return new d_List();
    d_NODE* curr = createNode(L->pHead->key);
    d_List* L1 = createList(curr);
    d_NODE* tmp = L->pHead->pNext;
    while (tmp != NULL) {
        addHead(L1, tmp->key);
        tmp = tmp->pNext;
    }
