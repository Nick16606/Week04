...
bool addPos(List*& L, int data, int pos)// Ở đây vị trí em dùng từ 0 đến n (giống index mảng), chèn sau node thứ pos - 1
{
...
void removePos(List*& L, int data, int pos)
{
    if (L->p_head == NULL)
        return;
    if (pos < 0 || pos > countElements(L)-1) return;
    else if (pos == 0) removeHead(L);
    else if (pos == countElements(L)-1) removeTail(L);
    else {
        NODE* prev=NULL;
        NODE* curr = L->p_head;
        for (int i = 0; i < pos; i++) {
        ...
        if (curr->key == data) {
            ...
            cout << "Khong phu hop"; // Bởi vì ở cái vị trí đó thì data không phải giá trị tại vị trí đó
        }
    }
    return;
}
...
List* reverseList(List* L)
{
    if (L->p_head == NULL && L->p_tail == NULL) { return new List(); }
    NODE* curr = createNode(L->p_head->key);
    List* L1 = createList(curr);
    NODE* tmp = L->p_head->p_next;
    while (tmp != NULL) {
        addHead(L1, tmp->key);
        tmp = tmp->p_next;
    }
